...
Options: -i  file  main bitstream (base or spatial enhancement layer)\n\
         -o  file  watermark report\n\
         \n");
    exit(0);
  }
  ...
  while(i < argc)
  {
    ...
    if(argv[i][0]=='-')
    {
      ...
      case 'I':
        ...
        if(NextArg || LastArg)
          printf("ERROR: -i must be followed the main bitstream filename\n");
        else
      ...
      case 'O':
        if(NextArg || LastArg)
          printf("ERROR: -o must be followed the main bitstream filename\n");
        else
          Output_Wtrkinfo_Filename = argv[++i]; 
